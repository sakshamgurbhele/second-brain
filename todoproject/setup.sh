...
# Create virtualenv inside todoproject
python3.12 -m venv "$DIR/todoenv"

# Install dependencies
"$DIR/todoenv/bin/pip" install --upgrade pip -q
"$DIR/todoenv/bin/pip" install -r "$DIR/requirements.txt" -q
...
cd "$DIR"
"$DIR/todoenv/bin/python" manage.py migrate --run-syncdb
